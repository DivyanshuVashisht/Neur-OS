...
static int pathparser_path_valid_format(const char* filename){
    int len = strnlen(filename, NEUROS_MAX_PATH);
...
static int pathparser_get_drive_by_path(const char** path){
    ...
    // Add 3 bytes to skip byte number 0:/ 1:/ 2:/
    *path += 3;
    return drive_no;
}
...
static const char* pathparser_get_path_part(const char** path){
    char* result_path_part = kzalloc(NEUROS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_dirctory_path){
    int res = 0;
    const char* tmp_path = path;
    struct path_root* path_root = 0;
    if (strlen(path) > NEUROS_MAX_PATH){
        goto out;
    }
