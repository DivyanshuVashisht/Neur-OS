...
struct filesystem* filesystems[NEUROS_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[NEUROS_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem(){
    int i = 0;
    for (i = 0; i < NEUROS_MAX_FILESYSTEMS; i++){
        if (filesystems[i] == 0){
...
static int file_new_descriptor(struct file_descriptor** desc_out){
    int res = -ENOMEM;
    for (int i = 0; i < NEUROS_MAX_FILE_DESCRIPTORS; i++){
...
static struct file_descriptor* file_get_descriptor(int fd){
    if (fd <= 0 || fd >= NEUROS_MAX_FILE_DESCRIPTORS){
        return 0;
    }
...
struct filesystem* fs_resolve(struct disk* disk){
    ...
    for (int i = 0; i < NEUROS_MAX_FILESYSTEMS; i++){
